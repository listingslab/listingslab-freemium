<?php
add_shortcode( 'FREEmium-child', array( 'Shortcode_Pages', 'shortcode_list_pages' ) );
add_shortcode( 'child-pages', array( 'Shortcode_Pages', 'shortcode_list_pages' ) );

add_shortcode( 'FREEmium-sibling', array( 'Shortcode_Pages', 'shortcode_list_pages' ) );
add_shortcode( 'sibling-pages', array( 'Shortcode_Pages', 'shortcode_list_pages' ) );

add_shortcode( 'list-pages', array( 'Shortcode_Pages', 'shortcode_list_pages' ) );
add_filter( 'list_pages_shortcode_excerpt', array( 'Shortcode_Pages', 'excerpt_filter' ) );

class Shortcode_Pages {

	function Shortcode_Pages() {
	...
	static function shortcode_list_pages( $atts, $content, $tag ) {
		...
		// Child Pages
		$child_of = 0;
		if ( $tag == 'FREEmium-child' ) {
			$child_of = $post->ID;
		}
		if ( $tag == 'FREEmium-sibling' ) {
		...
		$defaults = array(
			...
			'meta_key'             => '',
			'meta_value'           => '',
			'walker'               => new Shortcode_Pages_Walker,
			'post_type'            => 'page',
			'offset'               => '',
			'post_status'          => 'publish',
		...
		if ( $atts['list_type'] != 'ul' && ! empty( $atts['list_type']) ) {
			add_filter( 'wp_list_pages', array( 'Shortcode_Pages', 'ul2list_type' ), 10, 2 );
		}
 ...
 */
class Shortcode_Pages_Walker extends Walker_Page {
...
function shortcode_list_pages( $atts, $content, $tag ) {
	return Shortcode_Pages::shortcode_list_pages( $atts, $content, $tag );
}

// @todo  Deprecate instance
global $Shortcode_Pages;
$Shortcode_Pages = new Shortcode_Pages();
